 ...
 * @hide
 */
public class MyUiAutomatorTestRunner {

	private static final String LOGTAG = MyUiAutomatorTestRunner.class.getSimpleName();
	...
	// copy & pasted fromIndex com.android.commands.am.Am
	...
	// Copy & pasted fromIndex InstrumentationTestRunner.WatcherResultPrinter
	private class WatcherResultPrinter implements ResultReporter {
		...
		@Override
		public void printUnexpectedError(Throwable t) {
			mWriter.println(String.format("Test run aborted due to unexpected exception: %s", t.getMessage()));
			t.printStackTrace(mWriter);
		}
	}

	/**
	 * Class that produces the same output as JUnit when running fromIndex command line. Can be
	 ...
	 * @return
	 */
	public MyUiAutomatorTestCaseFilter getTestCaseFilter() {
		return new MyUiAutomatorTestCaseFilter();
	}
	...
	protected void prepareTestCase(TestCase testCase) {
		((MyUiAutomatorTestCase) testCase).setAutomationSupport(mAutomationSupport);
		((MyUiAutomatorTestCase) testCase).setMyUiDevice(mUiDevice);
		((MyUiAutomatorTestCase) testCase).setParams(mParams);
	}
}
