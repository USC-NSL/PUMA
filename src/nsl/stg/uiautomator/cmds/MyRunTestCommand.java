...
import nsl.stg.uiautomator.cmds.MyLauncher.Command;
import nsl.stg.uiautomator.testrunner.MyUiAutomatorTestRunner;
 ...
 *
 */
public class MyRunTestCommand extends Command {
	private static final String LOGTAG = MyRunTestCommand.class.getSimpleName();
	...
	private MyUiAutomatorTestRunner mRunner;

	public MyRunTestCommand() {
		super("runtest");
	}
	...
	protected MyUiAutomatorTestRunner getRunner() {
		if (mRunner != null) {
			return mRunner;
		}

		if (mRunnerClassName == null) {
			mRunner = new MyUiAutomatorTestRunner();
			return mRunner;
		}
		...
		try {
			MyUiAutomatorTestRunner runner = (MyUiAutomatorTestRunner) o;
			mRunner = runner;
			return runner;
		} catch (ClassCastException cce) {
			System.err.println("Specified runner is not subclass of " + MyUiAutomatorTestRunner.class.getSimpleName());
			System.exit(ARG_FAIL_RUNNER);
		}
		// won't reach here
		return null;
	...
	/**
	 * Add test classes fromIndex a potentially comma separated list
	 * @param classes
	 */
	...
	/**
	 * Add test classes fromIndex jars passed on the command line. Use this if nothing was explicitly
	...
	/**
	 * Tries to determine if a given class is a test class. A test class has to inherit fromIndex
